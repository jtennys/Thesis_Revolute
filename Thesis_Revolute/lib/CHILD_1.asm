...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_EnableInt
;
;  DESCRIPTION:
...
_CHILD_1_EnableInt:
   ...
   M8C_EnableIntMask CHILD_1_INT_REG, CHILD_1_bINT_MASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_DisableInt
;
;  DESCRIPTION:
...
_CHILD_1_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask CHILD_1_INT_REG, CHILD_1_bINT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_CHILD_1_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[CHILD_1_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_bReadRxData
;
;  DESCRIPTION:
...
;
 CHILD_1_bReadRxData:
_CHILD_1_bReadRxData:
 bCHILD_1_ReadRxData:
_bCHILD_1_ReadRxData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[CHILD_1_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_bReadRxStatus
;
;  DESCRIPTION:
...
;    functions.
;
 CHILD_1_bReadRxStatus:
_CHILD_1_bReadRxStatus:
 bCHILD_1_ReadRxStatus:
_bCHILD_1_ReadRxStatus:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[CHILD_1_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_cGetChar
;
;  DESCRIPTION:
...
.getChar_Loop:
   tst REG[CHILD_1_CONTROL_REG],CHILD_1_RX_REG_FULL   ; Check if a character is ready
   jz  .getChar_Loop                                        ; If not loop

   mov A, REG[CHILD_1_RX_BUFFER_REG]             ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;
...
_CHILD_1_cReadChar:
   ...
   mov  A,REG[CHILD_1_CONTROL_REG]                         ; Get Status of RX
   push A
   and  A,CHILD_1_RX_COMPLETE                              ; Check if a character is ready
   jnz  .RX_DATA_RDY                                       ; Data Ready go read it.
   pop  A
   jmp  .RX_NO_VALID_CHAR

.RX_DATA_RDY:
   mov  A,REG[CHILD_1_RX_BUFFER_REG]                       ; Read data first, then
   swap A,X                                                ; determine if data is valid

   pop  A                                                  ; Check for errors
   and  A,(CHILD_1_RX_PARITY_ERROR | CHILD_1_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_iReadChar
;
...
_CHILD_1_iReadChar:
   ...
   and  A,(CHILD_1_RX_ERROR|CHILD_1_RX_REG_FULL)
   push A
   and  A,CHILD_1_RX_COMPLETE                              ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,CHILD_1_RX_NO_DATA                               ; Add no data flag
   swap A,X
   jmp  End_CHILD_1_iReadChar
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_CmdReset
;
;  DESCRIPTION:
...
_CHILD_1_CmdReset:
   ...
   RAM_SETPAGE_CUR >CHILD_1_aRxBuffer
   mov [CHILD_1_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >ptrParam
   mov [ptrParam],0x00
   RAM_SETPAGE_CUR >CHILD_1_bRxCnt
   mov [CHILD_1_bRxCnt], 0x00
   and [CHILD_1_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_bCmdCheck
;
;  DESCRIPTION:
...
_CHILD_1_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >CHILD_1_fStatus
   mov A,  [CHILD_1_fStatus]
   and A, CHILD_1_RX_BUF_CMDTERM                 ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_bErrCheck
;
;  DESCRIPTION:
...
_CHILD_1_bErrCheck:
   ...
   RAM_SETPAGE_CUR >CHILD_1_fStatus
   mov A,  [CHILD_1_fStatus]
   and A, CHILD_1_RX_BUF_ERROR                   ; Mask off Error status
   and [CHILD_1_fStatus], ~CHILD_1_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_bCmdLength
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_szGetParam
;
;  DESCRIPTION:
...
;     CHILD_1_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, CHILD_1_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See CHILD_1_bCmdCheck).
;
...
_CHILD_1_szGetParam:
   ...
   mov  A, <CHILD_1_aRxBuffer               ; Get address to receive buffer
...
.CheckForDelim:
    cmp  A,CHILD_1_DELIMITER                ; Check if we have a delimiter
    jnz  .ParamStartFound
    inc  X                                  ; Increment both current pointer and
    inc  [ptrParam]                         ; stored pointer.
    mov  A,[X]                              ; Get character pointed by X
    cmp  [ptrParam],(CHILD_1_RX_BUFFER_SIZE -1)  ; Check if we are at the end of buffer
    jnz  .CheckForDelim
                                            ; End of string found
.EndOfString:
    push X                                  ; Save ptr
...
.ParamLoop:
    ...
    mov  [ptrParam],(CHILD_1_RX_BUFFER_SIZE -1)  ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A,>CHILD_1_aRxBuffer                     ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_1_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 CHILD_1_szGetRestOfParams:
_CHILD_1_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam
