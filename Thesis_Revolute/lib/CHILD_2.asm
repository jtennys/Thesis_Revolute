...
;  Global Symbols
;-----------------------------------------------
export   CHILD_2_EnableInt
export  _CHILD_2_EnableInt
export   CHILD_2_DisableInt
export  _CHILD_2_DisableInt
export   CHILD_2_Start
export  _CHILD_2_Start
export   CHILD_2_Stop
export  _CHILD_2_Stop
export   CHILD_2_bReadRxData
export  _CHILD_2_bReadRxData
export   CHILD_2_bReadRxStatus
export  _CHILD_2_bReadRxStatus

; Old function name convension, do not use.
; These will be removed in a future release.
export  bCHILD_2_ReadRxData
export _bCHILD_2_ReadRxData
export  bCHILD_2_ReadRxStatus
export _bCHILD_2_ReadRxStatus

;-----------------------------------------------
; High Level RX functions
;-----------------------------------------------

export  CHILD_2_cGetChar
export _CHILD_2_cGetChar
export  CHILD_2_cReadChar
export _CHILD_2_cReadChar
export  CHILD_2_iReadChar
export _CHILD_2_iReadChar

IF (CHILD_2_RXBUF_ENABLE)
export  CHILD_2_CmdReset
export _CHILD_2_CmdReset
export  CHILD_2_bCmdCheck
export _CHILD_2_bCmdCheck
export  CHILD_2_bCmdLength
export _CHILD_2_bCmdLength
export  CHILD_2_bErrCheck
export _CHILD_2_bErrCheck

export  CHILD_2_szGetParam
export _CHILD_2_szGetParam
export  CHILD_2_szGetRestOfParams
export _CHILD_2_szGetRestOfParams

;-----------------------------------------------
;  Variables
;-----------------------------------------------

AREA CHILD_2_RAM(RAM,REL,CON)
 ptrParam:   			BLK  1
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_EnableInt
;
;  DESCRIPTION:
...
_CHILD_2_EnableInt:
   ...
   M8C_EnableIntMask CHILD_2_INT_REG, CHILD_2_bINT_MASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_DisableInt
;
;  DESCRIPTION:
...
_CHILD_2_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask CHILD_2_INT_REG, CHILD_2_bINT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_CHILD_2_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[CHILD_2_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_bReadRxData
;
;  DESCRIPTION:
...
;
 CHILD_2_bReadRxData:
_CHILD_2_bReadRxData:
 bCHILD_2_ReadRxData:
_bCHILD_2_ReadRxData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[CHILD_2_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_bReadRxStatus
;
;  DESCRIPTION:
...
;    functions.
;
 CHILD_2_bReadRxStatus:
_CHILD_2_bReadRxStatus:
 bCHILD_2_ReadRxStatus:
_bCHILD_2_ReadRxStatus:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[CHILD_2_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_cGetChar
;
;  DESCRIPTION:
...
.getChar_Loop:
   tst REG[CHILD_2_CONTROL_REG],CHILD_2_RX_REG_FULL   ; Check if a character is ready
   jz  .getChar_Loop                                        ; If not loop

   mov A, REG[CHILD_2_RX_BUFFER_REG]             ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;
...
_CHILD_2_cReadChar:
   ...
   mov  A,REG[CHILD_2_CONTROL_REG]                         ; Get Status of RX
   push A
   and  A,CHILD_2_RX_COMPLETE                              ; Check if a character is ready
   jnz  .RX_DATA_RDY                                       ; Data Ready go read it.
   pop  A
   jmp  .RX_NO_VALID_CHAR

.RX_DATA_RDY:
   mov  A,REG[CHILD_2_RX_BUFFER_REG]                       ; Read data first, then
   swap A,X                                                ; determine if data is valid

   pop  A                                                  ; Check for errors
   and  A,(CHILD_2_RX_PARITY_ERROR | CHILD_2_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_iReadChar
;
...
_CHILD_2_iReadChar:
   ...
   and  A,(CHILD_2_RX_ERROR|CHILD_2_RX_REG_FULL)
   push A
   and  A,CHILD_2_RX_COMPLETE                              ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,CHILD_2_RX_NO_DATA                               ; Add no data flag
   swap A,X
   jmp  End_CHILD_2_iReadChar

.RX_GET_DATA:
   pop  A
   and  A,CHILD_2_RX_ERROR
   swap A,X
   mov  A,REG[CHILD_2_RX_BUFFER_REG]                       ; Read data first, then
                                                           ; determine if data is valid

 End_CHILD_2_iReadChar:
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_CmdReset
;
;  DESCRIPTION:
...
_CHILD_2_CmdReset:
   ...
   RAM_SETPAGE_CUR >CHILD_2_aRxBuffer
   mov [CHILD_2_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >ptrParam
   mov [ptrParam],0x00
   RAM_SETPAGE_CUR >CHILD_2_bRxCnt
   mov [CHILD_2_bRxCnt], 0x00
   and [CHILD_2_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_bCmdCheck
;
;  DESCRIPTION:
...
_CHILD_2_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >CHILD_2_fStatus
   mov A,  [CHILD_2_fStatus]
   and A, CHILD_2_RX_BUF_CMDTERM                 ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_bErrCheck
;
;  DESCRIPTION:
...
_CHILD_2_bErrCheck:
   ...
   RAM_SETPAGE_CUR >CHILD_2_fStatus
   mov A,  [CHILD_2_fStatus]
   and A, CHILD_2_RX_BUF_ERROR                   ; Mask off Error status
   and [CHILD_2_fStatus], ~CHILD_2_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_bCmdLength
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_szGetParam
;
;  DESCRIPTION:
...
;     CHILD_2_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, CHILD_2_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See CHILD_2_bCmdCheck).
;
...
_CHILD_2_szGetParam:
   ...
   RAM_SETPAGE_CUR >ptrParam
   RAM_SETPAGE_IDX >CHILD_2_aRxBuffer

   mov  A, <CHILD_2_aRxBuffer               ; Get address to receive buffer
...
.CheckForDelim:
    cmp  A,CHILD_2_DELIMITER                ; Check if we have a delimiter
    jnz  .ParamStartFound
    inc  X                                  ; Increment both current pointer and
    inc  [ptrParam]                         ; stored pointer.
    mov  A,[X]                              ; Get character pointed by X
    cmp  [ptrParam],(CHILD_2_RX_BUFFER_SIZE -1)  ; Check if we are at the end of buffer
    jnz  .CheckForDelim
                                            ; End of string found
.EndOfString:
    push X                                  ; Save ptr
...
.ParamLoop:
    ...
    mov  [ptrParam],(CHILD_2_RX_BUFFER_SIZE -1)  ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A,>CHILD_2_aRxBuffer                     ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: CHILD_2_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 CHILD_2_szGetRestOfParams:
_CHILD_2_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam
